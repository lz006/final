...
public class ReportImpl extends RemoteServiceServlet implements Report {
	...
	public String createStudentenplan(Semesterverband sv)
			throws RuntimeException {

		Vector<Belegung> svBelegungen = verwaltung.belegungMapper
				.findBySemesterverband(sv);

		if (svBelegungen == null || svBelegungen.size() <= 0) {
			throw new RuntimeException(
					"Es sind keine Belegungen zu diesem Semesterverband vorhanden");
		}
		...
		for (int i = 0; i < svBelegungen.size(); i++) {
			...
			tempSB.append(svBelegungen.elementAt(i).getLehrveranstaltung()
					.getBezeichnung());
			tempSB.append("<br />");
			...
			for (int j = 0; j < svBelegungen.elementAt(i).getDozenten().size() - 1; j++) {
				tempSB.append("Prof. ");
				tempSB.append(svBelegungen.elementAt(i).getDozenten()
						.elementAt(j).getVorname());
				tempSB.append(" ");
				tempSB.append(svBelegungen.elementAt(i).getDozenten()
						.elementAt(j).getNachname());
				tempSB.append("<br />");
			}

			tempSB.append("Prof. ");
			tempSB.append(svBelegungen
					.elementAt(i)
					.getDozenten()
					.elementAt(
							svBelegungen.elementAt(i).getDozenten().size() - 1)
					.getVorname());
			tempSB.append(" ");
			tempSB.append(svBelegungen
					.elementAt(i)
			...
			sPlan.getPlan().elementAt(columnPointer)
					.set(rowPointer, tempSB.toString());
...
//		htmlSV.append("</p>");
	...
	public Vector<Studiengang> auslesenAlleStudiengaengeOhneSVuLV()
			throws RuntimeException {
		return verwaltung.auslesenAlleStudiengaengeOhneSVuLV();
	}

	public Vector<Semesterverband> auslesenSemesterverbaendeNachStudiengang(
			Studiengang sg) throws RuntimeException {
	...
	public String createDozentenplan(Dozent dozent) throws RuntimeException {
		...
		Vector<Belegung> dozentBelegungen = verwaltung.belegungMapper
				.findByDozent(dozent);

		if (dozentBelegungen == null || dozentBelegungen.size() <= 0) {
			throw new RuntimeException(
					"Es sind keine Belegungen für diesem Dozent vorhanden");
		}
		...
		for (int i = 0; i < dozentBelegungen.size(); i++) {
			...
			if (dozentBelegungen.elementAt(i).getZeitslot().getId() <= 7) {
				columnPointer = 1;
				rowPointer = dozentBelegungen.elementAt(i).getZeitslot()
						.getId();
			}
			if (dozentBelegungen.elementAt(i).getZeitslot().getId() > 7
					&& dozentBelegungen.elementAt(i).getZeitslot().getId() <= 14) {
				columnPointer = 2;
				rowPointer = dozentBelegungen.elementAt(i).getZeitslot()
						.getId() - 7;
			}
			if (dozentBelegungen.elementAt(i).getZeitslot().getId() > 14
					&& dozentBelegungen.elementAt(i).getZeitslot().getId() <= 21) {
				columnPointer = 3;
				rowPointer = dozentBelegungen.elementAt(i).getZeitslot()
						.getId() - 14;
			}
			if (dozentBelegungen.elementAt(i).getZeitslot().getId() > 21
					&& dozentBelegungen.elementAt(i).getZeitslot().getId() <= 28) {
				columnPointer = 4;
				rowPointer = dozentBelegungen.elementAt(i).getZeitslot()
						.getId() - 21;
			}
			if (dozentBelegungen.elementAt(i).getZeitslot().getId() > 28
					&& dozentBelegungen.elementAt(i).getZeitslot().getId() <= 35) {
				columnPointer = 5;
				rowPointer = dozentBelegungen.elementAt(i).getZeitslot()
						.getId() - 28;
			}
			if (dozentBelegungen.elementAt(i).getZeitslot().getId() > 35
					&& dozentBelegungen.elementAt(i).getZeitslot().getId() <= 42) {
				columnPointer = 6;
				rowPointer = dozentBelegungen.elementAt(i).getZeitslot()
						.getId() - 35;
			}
			if (dozentBelegungen.elementAt(i).getZeitslot().getId() > 42
					&& dozentBelegungen.elementAt(i).getZeitslot().getId() <= 49) {
				columnPointer = 7;
				rowPointer = dozentBelegungen.elementAt(i).getZeitslot()
						.getId() - 42;
			}
			...
			StringBuffer tempSB = new StringBuffer();
			tempSB.append(dozentBelegungen.elementAt(i).getLehrveranstaltung()
					.getBezeichnung());
			tempSB.append("<br />");
			tempSB.append(dozentBelegungen.elementAt(i).getRaum()
					.getBezeichnung());
			tempSB.append("<br />");

			if (dozentBelegungen.elementAt(i).getSemesterverbaende() != null
					&& dozentBelegungen.elementAt(i).getSemesterverbaende()
							.size() > 0) {
				for (int j = 0; j < dozentBelegungen.elementAt(i)
						.getSemesterverbaende().size() - 1; j++) {
					tempSB.append(verwaltung
							.auslesenSemesterverband(
									dozentBelegungen.elementAt(i)
											.getSemesterverbaende()
											.elementAt(j)).elementAt(0)
							.getStudiengang().getKuerzel());
					tempSB.append(" ");
					tempSB.append(dozentBelegungen.elementAt(i)
							.getSemesterverbaende().elementAt(j).getJahrgang());
					tempSB.append("<br />");
				}
				...
				tempSB.append(" ");
				tempSB.append(dozentBelegungen
						.elementAt(i)
						.getSemesterverbaende()
						.elementAt(
								dozentBelegungen.elementAt(i)
										.getSemesterverbaende().size() - 1)
						.getJahrgang());
			}

			dPlan.getPlan().elementAt(columnPointer)
					.set(rowPointer, tempSB.toString());
...
//		htmld.append("</p>");
	...
	public String createRaumplan(Raum raum) throws RuntimeException {
		...
		Vector<Belegung> raumBelegungen = verwaltung.belegungMapper
				.findByRaum(raum);

		if (raumBelegungen == null || raumBelegungen.size() <= 0) {
			throw new RuntimeException(
					"Es sind keine Belegungen zu diesem Raum vorhanden");
		}
		...
		for (int i = 0; i < raumBelegungen.size(); i++) {
			...
			if (raumBelegungen.elementAt(i).getZeitslot().getId() > 7
					&& raumBelegungen.elementAt(i).getZeitslot().getId() <= 14) {
				columnPointer = 2;
				rowPointer = raumBelegungen.elementAt(i).getZeitslot().getId() - 7;
			}
			if (raumBelegungen.elementAt(i).getZeitslot().getId() > 14
					&& raumBelegungen.elementAt(i).getZeitslot().getId() <= 21) {
				columnPointer = 3;
				rowPointer = raumBelegungen.elementAt(i).getZeitslot().getId() - 14;
			}
			if (raumBelegungen.elementAt(i).getZeitslot().getId() > 21
					&& raumBelegungen.elementAt(i).getZeitslot().getId() <= 28) {
				columnPointer = 4;
				rowPointer = raumBelegungen.elementAt(i).getZeitslot().getId() - 21;
			}
			if (raumBelegungen.elementAt(i).getZeitslot().getId() > 28
					&& raumBelegungen.elementAt(i).getZeitslot().getId() <= 35) {
				columnPointer = 5;
				rowPointer = raumBelegungen.elementAt(i).getZeitslot().getId() - 28;
			}
			if (raumBelegungen.elementAt(i).getZeitslot().getId() > 35
					&& raumBelegungen.elementAt(i).getZeitslot().getId() <= 42) {
				columnPointer = 6;
				rowPointer = raumBelegungen.elementAt(i).getZeitslot().getId() - 35;
			}
			if (raumBelegungen.elementAt(i).getZeitslot().getId() > 42
					&& raumBelegungen.elementAt(i).getZeitslot().getId() <= 49) {
				columnPointer = 7;
				rowPointer = raumBelegungen.elementAt(i).getZeitslot().getId() - 42;
			}

			StringBuffer tempSB = new StringBuffer();
			tempSB.append(raumBelegungen.elementAt(i).getLehrveranstaltung()
					.getBezeichnung());
			tempSB.append("<br />");

			for (int j = 0; j < raumBelegungen.elementAt(i).getDozenten()
					.size() - 1; j++) {
				tempSB.append("Prof. ");
			...
			tempSB.append("Prof. ");
			tempSB.append(raumBelegungen
					.elementAt(i)
					.getDozenten()
					.elementAt(
							raumBelegungen.elementAt(i).getDozenten().size() - 1)
					.getVorname());
			tempSB.append(" ");
			tempSB.append(raumBelegungen
					.elementAt(i)
			...
			if (raumBelegungen.elementAt(i).getSemesterverbaende() != null
					&& raumBelegungen.elementAt(i).getSemesterverbaende()
							.size() > 0) {
				for (int j = 0; j < raumBelegungen.elementAt(i)
						.getSemesterverbaende().size() - 1; j++) {
					tempSB.append(verwaltung
							.auslesenSemesterverband(
									raumBelegungen.elementAt(i)
											.getSemesterverbaende()
											.elementAt(j)).elementAt(0)
							.getStudiengang().getKuerzel());
					tempSB.append(" ");
					tempSB.append(raumBelegungen.elementAt(i)
							.getSemesterverbaende().elementAt(j).getJahrgang());
					tempSB.append("<br />");
				}
			...
			rPlan.getPlan().elementAt(columnPointer)
					.set(rowPointer, tempSB.toString());
