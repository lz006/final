...
public class DozentTreeViewModel implements TreeViewModel {
	...
	public <T> NodeInfo<?> getNodeInfo(T value) {
		...
		if (value instanceof Studiengang && unterMenuLVzuSG) {
			...
			verwaltung.auslesenLehrveranstaltungenNachSG((Studiengang)value, new AsyncCallback<Vector<Lehrveranstaltung>>() {
				public void onFailure(Throwable t) {
					Window.alert(t.toString());
				}
	...
	public void addLehrveranstaltung(Lehrveranstaltung lv) {
		if (lehrveranstaltungDataProvider != null) {
			lehrveranstaltungDataProvider.getList().add(lehrveranstaltungDataProvider.getList().size(), lv);
			lehrveranstaltungDataProvider.refresh();
		}
	}
	...
	public void addStudiengang(Studiengang sg) {
		if (studiengangDataProvider != null) {
			studiengangDataProvider.getList().add(studiengangDataProvider.getList().size(), sg);
			studiengangDataProvider.refresh();
		}
	}
			...
			public void onOpen(OpenEvent<TreeNode> event) {
				...
				if (rootNode.isChildOpen(0)) {
					if (rootNode.setChildOpen(0, true).isChildOpen(1)) {
						if (rootNode.setChildOpen(0, true).setChildOpen(1, true).isChildOpen(0) && rootNode.setChildOpen(0, true).setChildOpen(1, true).setChildOpen(0, true).equals(event.getTarget())) {
							unterMenuLVzuSG = true;
							unterMenuSVzuSG = false;
						...
						if (rootNode.setChildOpen(0, true).setChildOpen(1, true).isChildOpen(4) && rootNode.setChildOpen(0, true).setChildOpen(1, true).setChildOpen(4, true).equals(event.getTarget())) {
							unterMenuLVzuSG = false;
							unterMenuSVzuSG = true;
						...
						if (rootNode.setChildOpen(0, true).setChildOpen(1, true).isChildOpen(5) && rootNode.setChildOpen(0, true).setChildOpen(1, true).setChildOpen(5, true).equals(event.getTarget())) {
							unterMenuLVzuSG = false;
							unterMenuSVzuSG = false;
