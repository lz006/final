...
public class BelegungForm extends VerticalPanel {
	...
	// Container für die einzelnen Widgets des TabPanels um diese einzeln
	// ansprechen zu k�nnen

	Vector<ListBox> lbv = new Vector<ListBox>();
	Vector<Button> bv = new Vector<Button>();

	// Pointer(Zeiger) für die Adressierung der einzelnen Widgets im
	// �ndern-TabPanel
	...
	public void addChangeHandlerToAnlegenTagListBox() {
		anlegenTagListBox.addChangeHandler(new ChangeHandler() {
			public void onChange(ChangeEvent event) {
				...
				} else if (anlegenTagListBox.getSelectedIndex() > 0
						&& anlegenUhrzeitListBox.getSelectedIndex() > 0) {
				...
				.auslesenAlleStudiengaengeOhneSVuLV(new AsyncCallback<Vector<Studiengang>>() {
					...
					public void onSuccess(Vector<Studiengang> result) {
						...
						for (Studiengang sG : result) {
							...
							if (anlegenMaske) {
								studiengangListBoxAnlegen.addItem(sG
										.getBezeichnung());
							}
						}
						DOM.setStyleAttribute(RootPanel.getBodyElement(),
	...
	public void ladenSemesterverbaende(boolean info) {
		...
		if (!info) {
			...
			if (studiengangListBox.getSelectedIndex() != 0) {
				verwaltung.auslesenSemesterverbaendeNachStudiengang(
						studiengaengeVectorForListBox
								.elementAt(studiengangListBox
										.getSelectedIndex() - 1),
		...
		if (info) {
			...
			if (studiengangListBoxAnlegen.getSelectedIndex() != 0) {
				verwaltung.auslesenSemesterverbaendeNachStudiengang(
						studiengaengeVectorForListBox
								.elementAt(studiengangListBoxAnlegen
										.getSelectedIndex() - 1),
						new AsyncCallback<Vector<Semesterverband>>() {
							public void onFailure(Throwable caught) {
								Window.alert(caught.getMessage());
							...
							public void onSuccess(Vector<Semesterverband> result) {
								if (result == null || result.size() == 0) {
									...
									for (Semesterverband sv : semesterverbaendeVectorForListBoxAnlegen) {
										semesterverbandListBoxAnlegen
												.addItem(sv.getJahrgang());
									}
									semesterverbandListBoxAnlegen
											.setEnabled(true);
								}
	...
	public void ladenBelegungen() {
		...
		if (semesterverbaendeVectorForListBox != null) {
			verwaltung.auslesenBelegungenNachSV(
					semesterverbaendeVectorForListBox
							.elementAt(semesterverbandListBox
									.getSelectedIndex() - 1),
							...
							if (result == null || result.size() == 0) {
								...
								if (!anlegenMaske) {
									...
									if (dozentenVectorforListBox != null) {
										anlegenDozent1ListBox
												.setSelectedIndex(0);
										anlegenDozent2ListBox
												.setSelectedIndex(0);
										anlegenDozent3ListBox
												.setSelectedIndex(0);
									}
								}
							}
	...
	public void fillContextTab() {
		...
		for (int i = 0; i < 7; i++) {
			...
			for (int j = AnfangsWochentagZaehler; j <= EndWochentagZaehler; j++) {
				int azStunde = zeitslotsVectorForListBox.elementAt(j)
						.getAnfangszeit() / 60;
				int azMinute = zeitslotsVectorForListBox.elementAt(j)
						.getAnfangszeit() % 60;
				int ezStunde = zeitslotsVectorForListBox.elementAt(j)
						.getEndzeit() / 60;
				int ezMinute = zeitslotsVectorForListBox.elementAt(j)
						.getEndzeit() % 60;
				...
				tempGrid.setWidget(gridRowZaehler + 1, 0,
						new Label(zeitAnzeige.toString()));
				gridRowZaehler++;
			}
	...
	public void ladenDozenten() {
		...
		if (!anlegenMaske) {
			verwaltung.auslesenDozentenNachZeitslot(
					svBelegungen.elementAt(focusBelegungPointer).getZeitslot(),
					new AsyncCallback<Vector<Dozent>>() {
						...
						public void onSuccess(Vector<Dozent> result) {
							...
							for (Dozent dozent : result) {
								lbv.elementAt(focusDozentListBoxPointer)
										.addItem(
												dozent.getNachname() + " "
														+ dozent.getVorname());
							}

							lbv.elementAt(focusDozentListBoxPointer).addItem(
									"kein Dozent");

							if (result != null && result.size() > 0) {
								if (focusOneOfThreeDozentListBoxPointer == 1) {
									lbv.elementAt(focusDozentListBoxPointer - 1)
											.setEnabled(true);
									lbv.elementAt(focusDozentListBoxPointer + 1)
											.setEnabled(true);
									lbv.elementAt(focusDozentListBoxPointer + 2)
											.setEnabled(true);
									...
									lbv.elementAt(focusDozentListBoxPointer - 3)
											.setEnabled(true);
									lbv.elementAt(focusDozentListBoxPointer - 2)
											.setEnabled(true);
									lbv.elementAt(focusDozentListBoxPointer - 1)
							...
							bv.elementAt(focusAendernButtonPointer).setEnabled(
									true);
							bv.elementAt(focusAendernButtonPointer + 1)
									.setEnabled(true);
		...
		if (anlegenMaske) {
			if (anlegenLVListBox.getSelectedIndex() == 0) {
				...
				verwaltung.auslesenDozentenNachLV(lvVectorforListBox
						.elementAt(anlegenLVListBox.getSelectedIndex() - 1),
						new AsyncCallback<Vector<Dozent>>() {
							public void onFailure(Throwable caught) {
								Window.alert(caught.getMessage());
								anlegenTagListBox.setEnabled(true);
							...
							public void onSuccess(Vector<Dozent> result) {
								...
								for (Dozent dozent : dozentenVectorforListBox) {
									anlegenDozent1ListBox.addItem(dozent
											.getNachname()
											+ " "
											+ dozent.getVorname());
									anlegenDozent2ListBox.addItem(dozent
											.getNachname()
											+ " "
											+ dozent.getVorname());
									anlegenDozent3ListBox.addItem(dozent
											.getNachname()
											+ " "
											+ dozent.getVorname());
								}
	...
	public void ladenRaeume() {
		...
		if (!anlegenMaske) {
			verwaltung.auslesenVerfuegbareRaeumeZuZeitslotuSV(svBelegungen
					.elementAt(focusBelegungPointer).getZeitslot(),
					svBelegungen.elementAt(focusBelegungPointer)
					...
					new AsyncCallback<Vector<Raum>>() {
						...
						public void onSuccess(Vector<Raum> result) {
							...
							for (Raum raum : result) {
								lbv.elementAt(focusRaumListBoxPointer).addItem(
										raum.getBezeichnung());
							}

							lbv.elementAt(focusRaumListBoxPointer + 1)
									.setEnabled(true);
							lbv.elementAt(focusRaumListBoxPointer + 2)
									.setEnabled(true);
							lbv.elementAt(focusRaumListBoxPointer + 3)
									.setEnabled(true);
		...
		if (anlegenMaske) {
			...
			if (anlegenTagListBox.getSelectedIndex() > 0
					&& anlegenUhrzeitListBox.getSelectedIndex() > 0) {
				zeitslotIndex = ((anlegenTagListBox.getSelectedIndex() - 1) * 7)
						+ anlegenUhrzeitListBox.getSelectedIndex();
			} else {
			...
			vSV.add(semesterverbaendeVectorForListBox
					.elementAt(semesterverbandListBox.getSelectedIndex() - 1));

			verwaltung.auslesenVerfuegbareRaeumeZuZeitslotuSV(
					zeitslotsVectorForListBox.elementAt(zeitslotIndex - 1),
					vSV, new AsyncCallback<Vector<Raum>>() {
						...
						public void onSuccess(Vector<Raum> result) {
							...
							for (Raum raum : raeumeVectorforListBox) {
								anlegenRaumListBox.addItem(raum
										.getBezeichnung());
							}
							anlegenRaumListBox.setEnabled(true);
	...
	public void ladenLehrveranstaltungen() {
		verwaltung
				.auslesenLehrveranstaltungenNachSV(
						semesterverbaendeVectorForListBox
								.elementAt(semesterverbandListBox
										.getSelectedIndex() - 1),
						studiengaengeVectorForListBox
							...
							public void onSuccess(
								...
								if (anlegenMaske) {
									...
									for (Lehrveranstaltung lv : lvVectorforListBox) {
										anlegenLVListBox.addItem(lv
												.getBezeichnung());
									}
									anlegenGrid.setWidget(1, 2,
											anlegenLVListBox);
								}
								ladenBelegungen();
							}
	...
	public void fillContentTab() {
		...
		for (int i = 0; i < svBelegungen.size(); i++) {
			...
			if (svBelegungen.elementAt(i).getZeitslot().getId() > 7
					&& svBelegungen.elementAt(i).getZeitslot().getId() <= 14) {
				gridPointer = 1;
				rowPointer = svBelegungen.elementAt(i).getZeitslot().getId() - 7;
			}
			if (svBelegungen.elementAt(i).getZeitslot().getId() > 14
					&& svBelegungen.elementAt(i).getZeitslot().getId() <= 21) {
				gridPointer = 2;
				rowPointer = svBelegungen.elementAt(i).getZeitslot().getId() - 14;
			}
			if (svBelegungen.elementAt(i).getZeitslot().getId() > 21
					&& svBelegungen.elementAt(i).getZeitslot().getId() <= 28) {
				gridPointer = 3;
				rowPointer = svBelegungen.elementAt(i).getZeitslot().getId() - 21;
			}
			if (svBelegungen.elementAt(i).getZeitslot().getId() > 28
					&& svBelegungen.elementAt(i).getZeitslot().getId() <= 35) {
				gridPointer = 4;
				rowPointer = svBelegungen.elementAt(i).getZeitslot().getId() - 28;
			}
			if (svBelegungen.elementAt(i).getZeitslot().getId() > 35
					&& svBelegungen.elementAt(i).getZeitslot().getId() <= 42) {
				gridPointer = 5;
				rowPointer = svBelegungen.elementAt(i).getZeitslot().getId() - 35;
			}
			if (svBelegungen.elementAt(i).getZeitslot().getId() > 42
					&& svBelegungen.elementAt(i).getZeitslot().getId() <= 49) {
				gridPointer = 6;
				rowPointer = svBelegungen.elementAt(i).getZeitslot().getId() - 42;
			}
			lbv.elementAt(lvListBoxPointer).clear();
			lbv.elementAt(lvListBoxPointer).addItem(
					svBelegungen.elementAt(i).getLehrveranstaltung()
							.getBezeichnung());
			for (Lehrveranstaltung lv : lvVectorforListBox) {
				lbv.elementAt(lvListBoxPointer).addItem(lv.getBezeichnung());
			}
			gridVector.elementAt(gridPointer).setWidget(rowPointer, 1,
					lbv.elementAt(lvListBoxPointer));

			lbv.elementAt(raumListBoxPointer).clear();
			lbv.elementAt(raumListBoxPointer).addItem(
					svBelegungen.elementAt(i).getRaum().getBezeichnung());

			lbv.elementAt(raumListBoxPointer).addFocusHandler(
					new FocusHandler() {
						...
						public void onFocus(FocusEvent event) {
							...
							String firstItem = lbv.elementAt(
									tempRaumListBoxPointer).getValue(0);
							lbv.elementAt(tempRaumListBoxPointer).clear();
							lbv.elementAt(tempRaumListBoxPointer).addItem(
									firstItem);

							lbv.elementAt(tempRaumListBoxPointer + 1)
									.setEnabled(false);
							lbv.elementAt(tempRaumListBoxPointer + 2)
									.setEnabled(false);
							lbv.elementAt(tempRaumListBoxPointer + 3)
									.setEnabled(false);

							bv.elementAt(tempAendernButtonPointer).setEnabled(
									false);
							bv.elementAt(tempAendernButtonPointer + 1)
									.setEnabled(false);
			...
			gridVector.elementAt(gridPointer).setWidget(rowPointer, 2,
					lbv.elementAt(raumListBoxPointer));

			lbv.elementAt(dozent1ListBoxPointer).clear();
			lbv.elementAt(dozent1ListBoxPointer).addItem(
					svBelegungen.elementAt(i).getDozenten().elementAt(0)
							.getNachname()
							+ " "
							+ svBelegungen.elementAt(i).getDozenten()
									.elementAt(0).getVorname());
			if (svBelegungen.elementAt(i).getDozenten().size() > 1) {
				...
				lbv.elementAt(dozent2ListBoxPointer).addItem(
						svBelegungen.elementAt(i).getDozenten().elementAt(1)
								.getNachname()
								+ " "
								+ svBelegungen.elementAt(i).getDozenten()
										.elementAt(1).getVorname());
			} else {
				lbv.elementAt(dozent2ListBoxPointer).clear();
				lbv.elementAt(dozent2ListBoxPointer).addItem("---");
			}
			if (svBelegungen.elementAt(i).getDozenten().size() > 2) {
				...
				lbv.elementAt(dozent3ListBoxPointer).addItem(
						svBelegungen.elementAt(i).getDozenten().elementAt(2)
								.getNachname()
								+ " "
								+ svBelegungen.elementAt(i).getDozenten()
										.elementAt(2).getVorname());
			} else {
				lbv.elementAt(dozent3ListBoxPointer).clear();
				lbv.elementAt(dozent3ListBoxPointer).addItem("---");
			}

			lbv.elementAt(dozent1ListBoxPointer).addFocusHandler(
					new FocusHandler() {
						...
						public void onFocus(FocusEvent event) {
							...
							String firstItem = lbv.elementAt(
									tempDozent1ListBoxPointer).getValue(0);
							lbv.elementAt(tempDozent1ListBoxPointer).clear();
							lbv.elementAt(tempDozent1ListBoxPointer).addItem(
									firstItem);

							lbv.elementAt(tempDozent1ListBoxPointer - 1)
									.setEnabled(false);
							lbv.elementAt(tempDozent1ListBoxPointer + 1)
									.setEnabled(false);
							lbv.elementAt(tempDozent1ListBoxPointer + 2)
									.setEnabled(false);
			...
			lbv.elementAt(dozent2ListBoxPointer).addFocusHandler(
					new FocusHandler() {
						...
						public void onFocus(FocusEvent event) {
							...
							String firstItem = lbv.elementAt(
									tempDozent2ListBoxPointer).getValue(0);
							lbv.elementAt(tempDozent2ListBoxPointer).clear();
							lbv.elementAt(tempDozent2ListBoxPointer).addItem(
									firstItem);

							lbv.elementAt(tempDozent2ListBoxPointer - 2)
									.setEnabled(false);
							lbv.elementAt(tempDozent2ListBoxPointer - 1)
									.setEnabled(false);
							lbv.elementAt(tempDozent2ListBoxPointer + 1)
									.setEnabled(false);
			...
			lbv.elementAt(dozent3ListBoxPointer).addFocusHandler(
					new FocusHandler() {
						...
						public void onFocus(FocusEvent event) {
							...
							String firstItem = lbv.elementAt(
									tempDozent3ListBoxPointer).getValue(0);
							lbv.elementAt(tempDozent3ListBoxPointer).clear();
							lbv.elementAt(tempDozent3ListBoxPointer).addItem(
									firstItem);

							lbv.elementAt(tempDozent3ListBoxPointer - 3)
									.setEnabled(false);
							lbv.elementAt(tempDozent3ListBoxPointer - 2)
									.setEnabled(false);
							lbv.elementAt(tempDozent3ListBoxPointer - 1)
									.setEnabled(false);
			...
			gridVector.elementAt(gridPointer).setWidget(rowPointer, 3,
					lbv.elementAt(dozent1ListBoxPointer));
			gridVector.elementAt(gridPointer).setWidget(rowPointer, 4,
					lbv.elementAt(dozent2ListBoxPointer));
			gridVector.elementAt(gridPointer).setWidget(rowPointer, 5,
					lbv.elementAt(dozent3ListBoxPointer));

			if (svBelegungen.elementAt(i).getSemesterverbaende().size() > 1) {
				bv.elementAt(aendernButtonPointer).setStyleName(
						"gwt-MultipleSVs-Button");
			}
			bv.elementAt(aendernButtonPointer).addClickHandler(
					new ClickHandler() {
						...
						public void onClick(ClickEvent event) {
							...
							if (lbv.elementAt(tempLvListBoxPointer)
									.getSelectedIndex() > 0) {
								svBelegungen
										.elementAt(tempBelegungPointer)
										.setLehrveranstaltung(
												lvVectorforListBox
														.elementAt(lbv
																.elementAt(
																		tempLvListBoxPointer)
																.getSelectedIndex() - 1));
								check1 = false;
							}
							if (lbv.elementAt(tempRaumListBoxPointer)
									.getSelectedIndex() > 0
									&& tempBelegungPointer == focusBelegungPointer) {
								svBelegungen
										.elementAt(tempBelegungPointer)
										.setRaum(
												raeumeVectorforListBox
														.elementAt(lbv
																.elementAt(
																		tempRaumListBoxPointer)
																.getSelectedIndex() - 1));
								check1 = false;
							}
							...
							if (lbv.elementAt(tempDozent1ListBoxPointer)
									.getSelectedIndex() > 0
									&& (!(lbv
											.elementAt(
													tempDozent1ListBoxPointer)
											.getValue(
													lbv.elementAt(
															tempDozent1ListBoxPointer)
															.getSelectedIndex())
											.equals("kein Dozent")))) {
								tempDozenten.add(dozentenVectorforListBox
										.elementAt(lbv.elementAt(
												tempDozent1ListBoxPointer)
												.getSelectedIndex() - 1));
								check1 = false;
							...
							if (lbv.elementAt(tempDozent1ListBoxPointer)
									.getSelectedIndex() == 0) {
								tempDozenten.add(svBelegungen
										.elementAt(tempBelegungPointer)
										.getDozenten().elementAt(0));
							...
							if (lbv.elementAt(tempDozent2ListBoxPointer)
									.getSelectedIndex() > 0
									&& (!(lbv
											.elementAt(
													tempDozent2ListBoxPointer)
											.getValue(
													lbv.elementAt(
															tempDozent2ListBoxPointer)
															.getSelectedIndex())
							...
							if (lbv.elementAt(tempDozent3ListBoxPointer)
									.getSelectedIndex() > 0
									&& (!(lbv
											.elementAt(
													tempDozent2ListBoxPointer)
							...
							svBelegungen.elementAt(tempBelegungPointer)
									.setDozenten(tempDozenten);

							if (!check1) {
								verwaltung.aendernBelegung(svBelegungen
										.elementAt(tempBelegungPointer),
										new AsyncCallback<Belegung>() {
											...
											public void onSuccess(
												...
												if (focusRaumListBoxPointer != null) {
													String tempFirstRaumItem = lbv
															.elementAt(
																	focusRaumListBoxPointer)
															.getValue(
																	lbv.elementAt(
																			focusRaumListBoxPointer)
																			.getSelectedIndex());
												...
												if (focusDozentListBoxPointer != null) {
													...
													if (focusOneOfThreeDozentListBoxPointer == 1) {
														tempFirstDozent1Item = lbv
																.elementAt(
																		focusDozentListBoxPointer)
																.getValue(
																		lbv.elementAt(
																				focusDozentListBoxPointer)
																				.getSelectedIndex());
														tempFirstDozent2Item = lbv
																.elementAt(
																		focusDozentListBoxPointer + 1)
																.getValue(
																		lbv.elementAt(
																				focusDozentListBoxPointer + 1)
																				.getSelectedIndex());
														tempFirstDozent3Item = lbv
																.elementAt(
																		focusDozentListBoxPointer + 2)
																.getValue(
																		lbv.elementAt(
																				focusDozentListBoxPointer + 2)
																				.getSelectedIndex());
			...
			gridVector.elementAt(gridPointer).setWidget(rowPointer, 6,
					bv.elementAt(aendernButtonPointer));

			bv.elementAt(loeschenButtonPointer).addClickHandler(
					new ClickHandler() {
						...
						public void onClick(ClickEvent event) {
							...
							verwaltung.loeschenBelegungen(svBelegungen
									.elementAt(tempBelegungPointer),
									semesterverbaendeVectorForListBox
											.elementAt(semesterverbandListBox
													.getSelectedIndex() - 1),
									new AsyncCallback<Void>() {
										...
										public void onSuccess(Void result) {
											...
											gridVector.elementAt(
													tempGridPointer).setWidget(
													tempRowPointer, 1, null);
											gridVector.elementAt(
													tempGridPointer).setWidget(
													tempRowPointer, 2, null);
											gridVector.elementAt(
													tempGridPointer).setWidget(
													tempRowPointer, 3, null);
											gridVector.elementAt(
													tempGridPointer).setWidget(
													tempRowPointer, 4, null);
											gridVector.elementAt(
													tempGridPointer).setWidget(
													tempRowPointer, 5, null);
											gridVector.elementAt(
													tempGridPointer).setWidget(
													tempRowPointer, 6, null);
											gridVector.elementAt(
													tempGridPointer).setWidget(
													tempRowPointer, 7, null);
			...
			gridVector.elementAt(gridPointer).setWidget(rowPointer, 7,
					bv.elementAt(loeschenButtonPointer));
	...
	public void fillContentTabAnlegen() {
		...
		for (int i = 0; i < svBelegungen.size(); i++) {
			...
			lvLabel.setText(svBelegungen.elementAt(i).getLehrveranstaltung()
					.getBezeichnung());
			gridVector.elementAt(gridPointer).setWidget(rowPointer, 1, lvLabel);

			Label raumLabel = new Label();
			raumLabel.setText(svBelegungen.elementAt(i).getRaum()
					.getBezeichnung());
			gridVector.elementAt(gridPointer).setWidget(rowPointer, 2,
					raumLabel);

			Label dozentLabel1 = new Label();
			dozentLabel1.setText(svBelegungen.elementAt(i).getDozenten()
					.elementAt(0).getNachname()
					+ " "
					+ svBelegungen.elementAt(i).getDozenten().elementAt(0)
							.getVorname());
			gridVector.elementAt(gridPointer).setWidget(rowPointer, 3,
			...
			if (svBelegungen.elementAt(i).getDozenten().size() > 1) {
				...
				dozentLabel2.setText(svBelegungen.elementAt(i).getDozenten()
						.elementAt(1).getNachname()
						+ " "
						+ svBelegungen.elementAt(i).getDozenten().elementAt(1)
								.getVorname());
				gridVector.elementAt(gridPointer).setWidget(rowPointer, 4,
			...
			if (svBelegungen.elementAt(i).getDozenten().size() > 2) {
				...
				dozentLabel3.setText(svBelegungen.elementAt(i).getDozenten()
						.elementAt(2).getNachname()
						+ " "
						+ svBelegungen.elementAt(i).getDozenten().elementAt(2)
								.getVorname());
				gridVector.elementAt(gridPointer).setWidget(rowPointer, 4,
		...
		if (info) {
			...
			hinzufuegenButtonAnlegen.addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					...
					if (semesterverbaendeVectorForListBoxAnlegen == null
							|| semesterverbaendeVectorForListBoxAnlegen.size() < 1) {
						Window.alert("Bitte wählen Sie zuerst einen Semesterverband");
					} else {
						boolean check = true;
						if (semesterverbaendeVectorForListBox == null
								|| semesterverbaendeVectorForListBox.size() < 1) {
						...
						if (check) {
							for (Semesterverband sv : SVvonNeuerBelegung) {
								if (sv.getId() == semesterverbaendeVectorForListBoxAnlegen
										.elementAt(
												semesterverbandListBoxAnlegen
														.getSelectedIndex())
										.getId()
										|| sv.getId() == semesterverbaendeVectorForListBox
												.elementAt(
														semesterverbandListBox
																.getSelectedIndex() - 1)
												.getId()) {
						...
						if (check) {
							if (semesterverbaendeVectorForListBoxAnlegen
									.elementAt(
											semesterverbandListBoxAnlegen
													.getSelectedIndex())
									.getId() == semesterverbaendeVectorForListBox
									.elementAt(
											semesterverbandListBox
													.getSelectedIndex() - 1)
									.getId()) {
						...
						if (check) {
							SVvonNeuerBelegung
									.addElement(semesterverbaendeVectorForListBoxAnlegen
											.elementAt(semesterverbandListBoxAnlegen
													.getSelectedIndex()));
							SGKuerzel.addElement(studiengaengeVectorForListBox
									.elementAt(
											studiengangListBoxAnlegen
													.getSelectedIndex() - 1)
									.getKuerzel());
							createFlexTable();
						}
	...
	public void createFlexTable() {
		...
		if (SVvonNeuerBelegung != null && SVvonNeuerBelegung.size() > 0) {
			for (int i = 0; i < SVvonNeuerBelegung.size(); i++) {
				...
				svTable.setWidget(row, 1, new Label(SVvonNeuerBelegung
						.elementAt(i).getJahrgang()));

				Button loeschenButton = new Button("X");
				loeschenButton.addClickHandler(new ClickHandler() {
					public void onClick(ClickEvent event) {

						int rowIndex = svTable.getCellForEvent(event)
								.getRowIndex();
	...
	public void belegungAnlegen() {
		...
		if ((anlegenDozent1ListBox.getSelectedIndex() <= 0)
				&& (anlegenDozent2ListBox.getSelectedIndex() <= 0)
		...
		vsv.addElement(semesterverbaendeVectorForListBox
				.elementAt(semesterverbandListBox.getSelectedIndex() - 1));
		if (SVvonNeuerBelegung != null) {
			for (Semesterverband sv : SVvonNeuerBelegung) {
				vsv.addElement(sv);
		...
		if (anlegenDozent1ListBox.getSelectedIndex() > 0) {
			vd.addElement(dozentenVectorforListBox
					.elementAt(anlegenDozent1ListBox.getSelectedIndex() - 1));
		}
		if (anlegenDozent2ListBox.getSelectedIndex() > 0) {
			vd.addElement(dozentenVectorforListBox
					.elementAt(anlegenDozent2ListBox.getSelectedIndex() - 1));
		}
		if (anlegenDozent3ListBox.getSelectedIndex() > 0) {
			vd.addElement(dozentenVectorforListBox
					.elementAt(anlegenDozent3ListBox.getSelectedIndex() - 1));
		}
		if (check) {
			verwaltung.anlegenBelegung(lvVectorforListBox
					.elementAt(anlegenLVListBox.getSelectedIndex() - 1),
					raeumeVectorforListBox.elementAt(anlegenRaumListBox
							.getSelectedIndex() - 1), zeitslotsVectorForListBox
							.elementAt(zeitslotIndex - 1), vd, vsv,
					new AsyncCallback<Belegung>() {
						public void onFailure(Throwable caught) {
							DOM.setStyleAttribute(RootPanel.getBodyElement(),
									"cursor", "default");
			...
			anlegenLVListBox.setEnabled(true);
			if (!(raeumeVectorforListBox == null || raeumeVectorforListBox
					.size() == 0)) {
				anlegenRaumListBox.setEnabled(true);
			}
			if (!(dozentenVectorforListBox == null || dozentenVectorforListBox
					.size() == 0)) {
				anlegenDozent1ListBox.setEnabled(true);
				anlegenDozent2ListBox.setEnabled(true);
				anlegenDozent3ListBox.setEnabled(true);
